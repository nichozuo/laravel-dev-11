### Dockerfile
```
FROM nexus.zzwb.cc:18444/zuogeus/nginx-php:8.2  
...
RUN composer dump-autoload && php artisan config:cache && php artisan route:cache && chmod -R 777 storage/logs && chmod -R 777 storage/framework
```
